...
def pitchfork_attack(url, body, payload_sets, headers_list=None, method="POST"):
	...
	for i in range(nb_requests):
		...
		start = time.time()
		r = requests.request(method, url, headers=headers, data=req_body)
		end = time.time()
		duration = end - start
...
if __name__ == "__main__":
	...
	pitchfork_attack(args.url, args.body, payload_sets, headers_list=args.header, method=args.method)
